(ns aws.main
  "Kotodama WASM entrypoint for the Aws clean-room actor (L4) — Clojure port.
  ...
  facts (`aws.<Entity>/<field>`); `*store*` is the in-memory materialization
...
(def entity-specs
  [{:entity "Ec2Instance"  :plural "ec2instances"  :id-prefix "aws_ec2"
    :fields [:instanceId :instanceType :state :privateIp :region] :required [:instanceType :state]
    :coerce {}                       :refs {}}
   {:entity "S3Bucket"     :plural "s3buckets"     :id-prefix "aws_s3b"
    :fields [:name :region :versioning]                :required [:name :region]
    :coerce {:versioning :bool}      :refs {}}
   {:entity "S3Object"     :plural "s3objects"     :id-prefix "aws_s3o"
    :fields [:bucket :key :sizeBytes :etag]           :required [:bucket :key]
    :coerce {:sizeBytes :int}        :refs {}}
   {:entity "LambdaFunction" :plural "lambdafunctions" :id-prefix "aws_lam"
    :fields [:name :runtime :memoryMb :timeoutSec]    :required [:name :runtime]
    :coerce {:memoryMb :int :timeoutSec :int} :refs {}}
   {:entity "DynamoTable"  :plural "dynamotables"  :id-prefix "aws_dyn"
    :fields [:name :partitionKey :itemCount :status]  :required [:name :partitionKey]
    :coerce {:itemCount :int}        :refs {}}
   {:entity "IamRole"      :plural "iamroles"      :id-prefix "aws_iam"
    :fields [:name :arn :policyRef]                   :required [:name :arn]
    :coerce {}                       :refs {}}
   {:entity "RdsInstance"  :plural "rdsinstances"  :id-prefix "aws_rds"
    :fields [:identifier :engine :instanceClass :status] :required [:identifier :engine]
    :coerce {}                       :refs {}}])
...
(defn emit-facts
  "EAVT facts for one record: {\"aws.<Entity>/<field>\" v ...}. The datomic
...
(defn healthz [] [{:status "ok" :actor "aws-compat" :tier tier :entities entities} 200])
